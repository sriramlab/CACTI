...
from src.modules.masker import MaskModule, CopySegmentModule
...
class CMAE(nn.Module):
    """
    Re-implementation of Copy Makser
    """
    def __init__(
        self,
        table_len=372,
        ...
        **kwargs
        ...
        ### Encoder
        self.tab_embed = TabEmbed(in_chans, embed_dim)
    ...
    def forward_encoder(self, x, mask):
        ...
        # x, restore_map, obs_mask = MaskModule.contextify(
        #     x, mask.type(torch.int32),
        #     shuffle = self.training
        #     ) # [B, L, D] -> [B, obs_len, D]
    ...
    def forward_decoder(self, latent, restore_map, obs_mask):
        # embed tokens
        ...
        # _x = MaskModule.decontextify(latent[:,1:,:], restore_map, self.mask_token)
    ...
    def forward(self, x, miss_map, copy_mask):
        ...
        # Get Decoder latent 
        xdecode = self.forward_decoder(latent, restore_map, obs_mask)
