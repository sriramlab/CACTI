...
class RMAE(nn.Module):
    ...
    def forward_encoder(self, x, miss_mask):
    ...
    def forward_decoder(self, latent, restore_map):
        # embed tokens
    ...
    def forward(self, x, miss_map):
	
        # Encode latent
        latent, restore_map, mask, nask = self.forward_encoder(x, miss_map)

        # Get Decoder latent 
        xdecode = self.forward_decoder(latent, restore_map)
