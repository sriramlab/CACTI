...
class CACTImodel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.ctx_dim = int((1-ctx_prop) * self.embed_dim)
        self.val_dim = int(self.embed_dim - self.ctx_dim)
        ...
        ### Encoder
        self.info_embed_enc = nn.Linear(self.context_embed_dim, self.ctx_dim)
        self.tab_embed = TabEmbed(in_chans, self.val_dim)
        ...
        self.info_embed_dec = nn.Linear(self.context_embed_dim, self.ctx_dim)
        self.project_decoder = nn.Linear(self.decoder_embed_dim, self.val_dim)
    ...
    def forward_loss(self, data, pred, miss_mask, recon_mask):
        ...
        if self.loss_type == "obs":
            loss = (loss * recon_mask).sum() / recon_mask.sum()    
        elif self.loss_type == "msk":
            loss = (loss * imp_mask).sum() / imp_mask.sum()        
