...
from src.models.cactilabllossctx import CACTImodel
...
class CACTIabl:
    def __init__(self, args, **kwargs):
        print(f"CACTIabl v{__version__}")
        ...
        ## loss type and ctx prop
        self.loss_type = args.loss_type
        self.ctx_prop = args.ctx_prop

        print(f"Running CACTI in loss type: {self.loss_type} MODE and CTX prop == {self.ctx_prop}")
    ...
    def fit(self, obs_data: torch.Tensor, resume_from_checkpoint=False):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            if epoch == self.epochs - 1 :
                ...
                print(f"#RTSATS# \n#RTSATS# : {self.data_info} : Epoch runtime : {epoch_runtime:.4f} seconds")
                peak_mem = torch.cuda.max_memory_allocated()
                peak_mem_gb = peak_mem / (1024**3)  # Convert to GB
                print(f"#RTSATS# : {self.data_info} : Peak GPU memory : {peak_mem_gb:.4f} GB")
    ...
    def transform(self, obs_data: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            infer_runtime = time.time() - infer_start_time
            print(f"#RTSATS# : {self.data_info} : Infer runtime : {infer_runtime:.4f} seconds")
